...
export class RentalEntitiesMongoRepository implements RentalEntitiesRepository {
    constructor(
        @Inject('RENTAL_ENTITY_MODEL') private readonly rentalEntityDocumentModel: Model<RentalEntityDocument>
    ) {}
    ...
    async delete(name: string): Promise<RentalEntityDocument> {
        return this.rentalEntityDocumentModel.findOneAndDelete({ name: name }).exec();
    }

    async updateStatus(permit: string, permit_status: string): Promise<RentalEntityDocument> {
        return this.rentalEntityDocumentModel
            .findOneAndUpdate({ permit: permit }, { permit_status: permit_status }, { new: true })
            .exec();
    }
}
